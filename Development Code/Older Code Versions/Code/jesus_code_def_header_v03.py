...
# Load the MIDI file
#path = r"C:\Users\User\Desktop\A_UMICH\SEM1\EECS 373\final proj\C_major_scale.mid"
path = r"C:\Users\User\Desktop\A_UMICH\SEM1\EECS 373\final proj\Christmas_Song_modified.mid"
#path = r"C:\Users\User\Desktop\A_UMICH\SEM1\EECS 373\final proj\Rollinginthedeep.mid"
...
for i in range(int(len(title)/5)):
    ...
    send_message(title[i*5 + 0],title[i*5 + 1],title[i*5 + 2],title[i*5 + 3],title[i*5 + 4])

extra = []
#send title 2
for a in range(len(title)%5):
    extra.append(title[int(len(title)/5)*5 + a])

for a in range(5 - (len(title)%5)):
    extra.append(0)
    
send_message(extra[0], extra[1], extra[2], extra[3], extra[4])
