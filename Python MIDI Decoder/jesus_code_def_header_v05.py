...
port = serial.Serial(port="COM52", baudrate=9600, bytesize=8, parity=serial.PARITY_NONE, stopbits=1, timeout=1)
...
while(sel_data != []):
    ...
    for d in sel_data:
        ...
        if((d[1] == item_st[1])): #we found matching note
            item_end = d
...
for a in def_data:
    ...
    note = int(a[0])
    start_tim = int(a[1]*100)
    stop_tim = int(a[2]*100)
